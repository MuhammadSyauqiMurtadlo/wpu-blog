...
<form class="mb-8 max-w-md mx-auto">
    ...
    <div class="relative">
        ...
        <input type="search" id="search" class="block w-full p-3 ps-9 bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand shadow-xs placeholder:text-body" placeholder="Search post titles..." autofocus autocomplete="off" name="search" />
    ...
    {{ $posts->links() }}
      <div class="mt-4 grid gap-8 lg:grid-cols-3 md:grid-cols-2">
              ...
              <div class="flex justify-between items-center">
                <a href="/posts?author={{ $post->author->username }}">
                  <div class="flex items-center space-x-4">
                      <img class="w-7 h-7 rounded-full" src="{{ $post->author->avatar ? asset('storage/' . $post->author->avatar) : asset('img/default-avatar.jpg') }}" alt="{{ $post->author->name }}" />
        ...
        <div>
          ...
          <a href="/posts" class="block font-medium text-blue-500 hover:underline">&laquo; Back to all Posts</a>
        </div>
        @endforelse
      </div>
  </div>
